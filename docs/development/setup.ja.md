...
.venv/bin/python -B -m atlantis_cli links
...
.venv\Scripts\python.exe -B -m atlantis_cli links
...
5. `Atlantis: doctor`、`Atlantis: Markdownリンク検査（offline）`、`Atlantis: unit test`を実行する。
...
`atlantis links`は、repository内のMarkdown path、画像、GitHub形式の見出しanchorをnetworkなしで検査します。
外部URLは件数だけを分離し、このcommandでは取得しません。

```bash
.venv/bin/python -B -m atlantis_cli links --json
```

外部ページの現在状態は、ユーザーが許可した手元browser MCPまたは将来のbrowser containerで別に観測します。
手元browserで開けたことを安全性保証へ昇格せず、使用browser、取得URL、final URL、revision／hash照合、
clock calibration、SaaS contextへ返した範囲をreceiptへ残します。

backendの初回選択、同権限内のfailover、利用者選定のhost検疫・防御デッキ、SaaS contextとの境界は
[Web原典観測・手元ブラウザ退避契約](browser-source-observation.ja.md)を参照してください。
