...
PLIの通常起動は`起動モード: Prompt Engineering Edition`と利用可能入口を先に示します。Python、container、
standalone runtime等を実行していないという詳細は、それらのoperationが要求された時に該当境界だけを
表示します。既定Helpは`AVAILABLE-NOW`の`summary`であり、未実装を消したわけではありません。
`help --detail all`、`capabilities`、`--state`で全状態を明示取得できます。
...
- 既定Helpが利用可能入口を先に示し、明示要求なしに未実装境界を前面化しない
- `--detail all`と`capabilities`では未実装・未試験・資源待ちを省略しない
