...
Atlantis implementation revision `beeb87f`に対する全local検証と、`d2bf18d`に対するtargeted検証:

```text
unit tests                 65 / 65 OK
doctor contract checks     pass
doctor overall             warn
markdown local links       49 files / 93 local references / failures 0
CORN validator             pass
...
git diff --check           pass
clean-room tracked HEAD     beeb87f / unit tests OK / exit 0
Help / doctor targeted      d2bf18d / 7 tests OK
final full / clean-room     pending after documentation receipt commit
```
