# Fold深度・減圧DeFold・Agency driftと棚別デバッグ研究ノート
...
共同操作、緊急介入では、暗黙のUser Profileが見えないContext断絶を作り得る。減圧DeFoldではsystem stateだけで
なく、どの設定・学習済み操作・個人化前提を解除、保持、説明するかも対象になる。

### I-04. Overrideだけでは安全にならず、減圧DeFoldが要る

ここで`DeFold`は、畳む前の構造へ破壊的に巻き戻す`Unfold`やunmountではない。Fold状態と来歴を保持したまま、
低G、人間可読、または引受主体が操作可能なPresentationへ段階的に展開renderingする処理である。完全な逆変換を
暗黙にせず、圧縮・選別・欠損・Agency移管とreceiptもrendering対象に含める。OS3x/4xから継承する用語は
`DeFold`であり、本ノートが置いた`Unfold`表記は訂正する。

```text
UnFoldの例:
  画像 -> text -> vector -> text -> 生成AI画像
  生成AI画像を採用し、それ以前の表現を破棄する置換系

DeFoldの例:
  保持されたtext anchor -> 画像・UI・説明・低G操作面へrendering
  anchorと来歴を残したままPresentationを展開する非破壊系
```
...
別事故を作り得る。`Emergency Stop`、`Human Override`、`Graceful Degradation`、`Controlled DeFold`、
...
- `Controlled DeFold`と`Decompression Handover`のmachine contract: `unknown / NOT IMPLEMENTED`
...
- `docs/architecture/`: G/D observability、Controlled DeFold、Agency driftの実装実験が成立した部分
