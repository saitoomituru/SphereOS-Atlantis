...
### F-11. 分類変更は道徳的な誤りの確定ではない

ユーザーは、定規が変更された場合に生じるのは、正確には「OAEの解釈定規を変更したOAE」と、その定規で対象OAEを再評価した「再評価OAE」のログであり、旧分類を「悪い誤り」として消すことではない、と補足した。

- 分類変更は、使用したWorld Config、Chunk Config、定規revision、判断時刻にscopeされる。
- `誤分類`、`違法`、`悪`、`危険`、`異端`等は同義ではない。
- 遡及判断を一律禁止せず、解釈定規変更OAEと、過去OAEを対象に現在時点で発行した再評価OAEとして記憶する。
- OAE system自体には、過去OAEを更新、置換、削除、過去時刻へ差し戻す遡及mutationを許さない。
- 解釈定規を変更するOAEと、その新定規で過去OAEを再評価するOAEをappendする自由は保持する。
- 新しい分類が採用されても、「当時その分類／合意が存在した」という履歴は残す。
- 過去の同時点OAEを新造することと、現在から過去を再判断することを分離する。
- 現在の定規だけを過去へ無標識に投影すると、当時の辺境、少数派、frontierを短絡的に悪へ分類するriskがある。
- 現在の合法分類を使って、過去に観測されたpain、被害、異議を消す逆方向のriskもある。
...
### I-04. 解釈定規変更OAEと再評価OAEとしてforkする

分類が変わったことを、直ちに「昔のFactが誤りだった」とは扱わない。定規のrevision、World Config、Chunk Config、適用scopeが変われば、同じ対象へ別種類の分類が出るのは正常なeffectである。また「誤り」と「悪」は別であり、ある分類を悪、違法、危険、異端、noise等へ写像するかは、適用した定規側の出力である。

既存分類への再判断が必要になった場合、source historyや過去OAEの発行時刻を書き換えず、まず現在時点の`Interpretation Ruler Change OAE`候補を発行する。その定規revisionを入力にして、対象OAEへの`Re-evaluation OAE`候補を別途発行する。過去を対象にした場合もOAE種別は再評価であり、`assessment_time_basis: retrospective`等で遡及評価であることを表す。

不変条件はappend-only lineageである。

```text
past OAE: immutable
ruler-change OAE: appends a new interpretation-ruler revision and scope
re-evaluation OAE: applies that revision to a target OAE
history: retains the past OAE, ruler-change OAE, and re-evaluation OAE
```

ここで`supersede`は現在の処理でどの分類を採用するかというpointer変更であり、旧OAEの消去ではない。OAE systemが過去合意を直接mutationできると、後世の定規に不都合な合意や異議の隠滅が可能になるため、Coreの権限外とする。

```text
Source Event / Observation OAE
├─ Classification OAE / World A / Ruler r1 / judged at T1
│  └─ Fact Adoption A@r1 / valid in scope S1
├─ Interpretation Ruler Change OAE / r1 -> r2 / emitted at T2
│  └─ declares new ruler revision, effective interval, and scope
├─ Re-evaluation OAE / target: original OAE / ruler: r2 / emitted after T2
│  └─ emits a new evaluation result without erasing r1 result
├─ Re-evaluation OAE / assessment_time_basis: retrospective
│  └─ current evaluation targets the past; target time remains unchanged
├─ World B / Classification B@r1
└─ World C / unresolved -> ⊥ + Last Order
```

時間軸は少なくとも次を分離する。

- `target_occurred_at`: 判断対象のeventが起きた時刻
- `source_recorded_at`: Sourceが記録された時刻
- `ruler_change_oae_emitted_at`: 解釈定規変更OAEを発行した時刻
- `evaluation_oae_emitted_at`: 分類／再評価OAEを発行した時刻
- `ruler_revision`: 使用した定規revision
- `ruler_effective_interval`: その定規が有効とされる期間
- `assessment_time_basis`: 当時定規、現在定規、別World定規、反実仮想のどれで測ったか
- `retroactive_effect_policy`: 再判断のeffectを過去へ及ぼすか

ここで、現在の定規を使って過去を評価する`retrospective assessment`と、制度上の権利、制裁、補償、正史等へ遡及的な効力を主張する`retroactive effect`を分離する。前者は解釈定規変更OAEに続く現在時点の再評価OAEとして記録できる。後者を許すか、禁止するか、限定するかはWorld Config／Chunk Configのpolicyである。ただし、そのpolicyが遡及効を採用しても、Atlantis Coreが過去OAEをmutationすることはない。現在時点のeffect OAEまたはFact Assertionが、過去の対象期間を参照する形でappendされる。

たとえば「当時の定規では合法、現在の定規では違法」と「現在の定規を当時にも遡及適用して違法だったことにする」は別claimである。逆方向の「当時は違法、現在は合法」も同様に、当時評価、現在評価、現在から見た過去評価、遡及effectを別artifactにする。

禁止するのは、現在発行した解釈定規変更OAEまたは再評価OAEを過去時点に存在したOAEとしてbackfillすること、または過去の分類を黙って置換することである。遡及評価そのものは、両OAEの発行時刻、対象OAE、使用定規を保持する限り許容できる。

分類scopeの拡大にも別のOAEを要求する。ある行為が現在定規で違法と分類されたことから、人物、所属集団、全人生、過去の全行為まで違法または悪だとすることは自動推論できない。逆に、ある行為が現在合法になったことから、当時観測されたpain、被害、違法分類、異議、救済請求まで無効にはならない。`legal_classification`、`moral_classification`、`harm_observation`、`person_identity`、`life_history`は別Dimensionであり、横断には根拠、authority、scopeを持つ新規OAEが要る。
...
- `target_occurred_at`、`ruler_change_oae_emitted_at`、`evaluation_oae_emitted_at`、`ruler_effective_interval`を混同せず記録できるか。
- 当時定規による分類と、解釈定規変更OAEに続く現在の再評価OAEを両方引き出せるか。
- 遡及判断と遡及effect policyを検索時に区別できるか。
...
機械拘束は固定binaryへ焼き切らず、signed／versioned Constraint Profileとして読み込む。runは使用revisionをpinし、後日のprofile更新で過去runを無標識に再解釈しない。再評価する場合は、Interpretation Ruler Change OAEで新revisionとscopeを発行し、そのrevisionを使うRe-evaluation OAEを過去runに対して明示発行する。
...
14. 当時合法／現在違法を、二つの定規revisionと二つの判断時刻で共存させる。
15. 当時違法／現在合法を、過去分類の消去なしに解釈定規変更OAE＋再評価OAEで表す。
16. 現在定規による過去の再評価と、過去へeffectを及ぼす遡及適用を別OAEにする。
17. frontierという位置分類を、悪という道徳分類へ自動変換しない。
...
11. **現在定規の遡及投影**: 現在の法、科学、文化、market narrativeだけで過去を再分類し、当時のfrontierや少数派を悪へ短絡しない。当時定規による評価と現在からの評価を並置する。
...
commit、log、artifact、後日の証言から、当時のObserver、Agency role、Intent、合意を遡及生成しない。後日の解析で定規を変える場合は後日時点のInterpretation Ruler Change OAEを発行し、その定規を適用したRe-evaluation OAEを、元World／元Instance Ghostを変えないbranchへ置く。

したがって`historical-oae-unavailable`は「過去を現在から判断してはならない」という禁止ではない。「当時存在したOAEを参照できないので、現在の判断を当時のOAEとして偽装してはならない」という停止条件である。解釈定規変更OAEと再評価OAEの発行時刻、対象時刻、使用定規、有効期間、遡及effect policyを保存すれば、過去OAEを対象にした再評価は表現できる。
...
- `ChunkConfig`の正式責務、およびWorld Configとの包含／override関係は未確定。
- Interpretation Ruler Change OAE、Re-evaluation OAE、`retroactive_effect_policy`の正式名称とschemaは未確定。
- 遡及effectを許すauthority、対象期間、救済、異議申立て、World branch規則は未確定。
...
3. 解釈定規変更OAE、再評価OAE、遡及評価、遡及effectの時間軸分離contract
4. Agency Capability、reach、authority、Port、actuationの分離contract
5. Infinite Coreの責務境界と`⊥`／Last Order契約
6. dynamic Constraint Profileとtrace replayの昇格手続
7. Data Prefab共通envelope
8. World Build compilerのsource／provenance境界
9. IBDへ渡す可逆保存・検索acceptance contract
