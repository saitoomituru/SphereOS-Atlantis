...
第三者のコード、文章、画像、音声、キャラクター、商標等を含める場合は、aliasだけの参照とpayload利用を
分離し、分かる範囲で出典、宣言license、変更、配布scopeを記録してください。分からない値を
架空の許諾番号で埋めず`unknown`とします。

Atlantis coreとlineage validatorは、個別素材の権利を認証せず、利用可または全面禁止を自動裁定しません。
公開配布へ何を含めるかは、その配布を行うcontributor／maintainer／World operatorが対象scopeで判断します。
Origin repositoryが公開routeをunmount／replaceしても、それは全Worldへのrights verdictではありません。

local-only／private assetはpublic repositoryの欠損ではなく、公開backfillを要求しません。
商用・社内extensionは対象App／integrator／Worldへ局所化し、Atlantis core、既存commons、無関係なWorld、
upstream fork権へproprietary scopeを伝播させません。
